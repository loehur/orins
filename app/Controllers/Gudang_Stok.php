...
class Gudang_Stok extends Controller
{
   ...
   public function print()
   {
      $data['stok'] = $this->data('Barang')->stok_data_list_all($this->userData['id_toko']);
      $data['stok_gudang'] = $this->data('Barang')->stok_data_list_all(0);
      $data['barang'] = $this->db(0)->get_cols_where('master_barang', "id, code_myob, grup, tipe, brand, model, product_name, CONCAT(grup,'-','tipe','-','brand') AS gr", 'en = 1 ORDER BY grup ASC, tipe ASC, brand ASC, CONCAT(model, product_name) ASC', 'gr', 1);
