...
class Buka_Order extends Controller
{
   ...
   public function content($parse = "")
   {
      ...
      if (isset($_SESSION['edit'][$this->userData['id_user']])) {
         ...
         $whereBarang = "(ref = '" . $dEdit[0] . "' AND stat <> 2) OR (id_sumber = " . $this->userData['id_toko'] . " AND user_id = " . $this->userData['id_user'] . " AND jenis = 2 AND id_target = 0 AND stat <> 2 AND stok = 0)";
      } else {
         $where = "id_toko = " . $this->userData['id_toko'] . " AND id_user = " . $this->userData['id_user'] . " AND id_pelanggan = 0 AND cancel = 0";
         $whereBarang = "id_sumber = " . $this->userData['id_toko'] . " AND user_id = " . $this->userData['id_user'] . " AND jenis = 2 AND id_target = 0 AND stat <> 2 AND stok = 0";
      }
