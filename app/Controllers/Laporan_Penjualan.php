...
class Laporan_Penjualan extends Controller
{
   ...
   public function cek_rekap_1()
   {
      ...
      if ($jumlahHari < 32) {
         ...
         $where = "(id_toko = " . $this->userData['id_toko'] . " OR id_afiliasi = " . $this->userData['id_toko'] . ") AND id_pelanggan <> 0 AND (insertTime BETWEEN '" . $from . "' AND '" . $to . "') GROUP BY id_produk ORDER BY id_order_data DESC";
   ...
   public function cek_rekap_2()
   {
      ...
      if ($jumlahHari < 32) {
         ...
         $where2 = "(id_sumber = " . $this->userData['id_toko'] . ") AND id_target <> 0 AND jenis = 2 AND (insertTime BETWEEN '" . $from . "' AND '" . $to . "') GROUP BY id_barang ORDER BY id DESC";
